 ...
 *
 *
 * 参考https://github.com/ashLikun/XTabLayout；
 * <p>
...
public class WTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity用于填充{@link WTabLayout}尽可能多。此选项仅生效
    ...
    /**
     * 重力用于在{@link WTabLayout}的中心布局选项卡。
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public WTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.WTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.WTabLayout_xTabIndicatorHeight, dpToPx(2)));
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.WTabLayout_xTabIndicatorWidth, 0));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.WTabLayout_xTabIndicatorColor, 0));
        mTabStrip.setSelectedIndicatorDrawable(a.getDrawable(R.styleable.WTabLayout_xTabIndicatorDrawable));
        mTabStrip.setWidthText(a.getBoolean(R.styleable.WTabLayout_xTabIndicatorWidthWidthText, false));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.WTabLayout_xTabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabPaddingBottom,
                mTabPaddingBottom);


        xTabTextAllCaps = a.getBoolean(R.styleable.WTabLayout_xTabTextAllCaps, false);

        mTabTextAppearance = a.getResourceId(R.styleable.WTabLayout_xTabTextAppearance,
                R.style.TextAppearance_Design_Tab);
        mTabTextSize = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabTextSize, 0);
        xTabTextBold = a.getBoolean(R.styleable.WTabLayout_xTabTextBold, false);

        xTabTextSelectedBold = a.getBoolean(R.styleable.WTabLayout_xTabTextSelectedBold, false);
        ...
        if (a.hasValue(R.styleable.WTabLayout_xTabSelectedTextSize)) {
            mTabSelectedTextSize = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabSelectedTextSize, 0);
        } else {
            mTabSelectedTextSize = mTabTextSize;
        }
        if (a.hasValue(R.styleable.WTabLayout_xTabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.WTabLayout_xTabTextColor);
        }

        if (a.hasValue(R.styleable.WTabLayout_xTabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.WTabLayout_xTabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        xTabDisplayNum = a.getInt(R.styleable.WTabLayout_xTabDisplayNum, 0);
        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabMaxWidth,
                INVALID_WIDTH);
        if (a.hasValue(R.styleable.WTabLayout_xTabItemBackground)) {
            xTabItemBackground = a.getDrawable(R.styleable.WTabLayout_xTabItemBackground);
        }
        if (a.hasValue(R.styleable.WTabLayout_xTabItemSelectedBackground)) {
            xTabItemSelectedBackground = a.getDrawable(R.styleable.WTabLayout_xTabItemSelectedBackground);
        }

        mContentInsetStart = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabContentStart, 0);
        mMode = a.getInt(R.styleable.WTabLayout_xTabMode, MODE_AUTO);
        mTabGravity = a.getInt(R.styleable.WTabLayout_xTabGravity, GRAVITY_FILL);

        dividerWidth = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabDividerWidth, 0);
        dividerHeight = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabDividerHeight, 0);

        dividerColor = a.getColor(R.styleable.WTabLayout_xTabDividerColor, Color.BLACK);
        dividerGravity = a.getInteger(R.styleable.WTabLayout_xTabDividerGravity, DividerDrawable.CENTER);

        iconAndTextSpace = a.getDimensionPixelSize(R.styleable.WTabLayout_xTabIconAndTextSpace, dpToPx(DEFAULT_GAP_TEXT_ICON));
        a.recycle();
...
//        invalidate();
    ...
    /**
     * @deprecated Use {@link #addOnTabSelectedListener(WTabLayout.OnTabSelectedListener)} and
     * {@link #removeOnTabSelectedListener(WTabLayout.OnTabSelectedListener)}.
     */
    @Deprecated
    public void setOnTabSelectedListener(@Nullable WTabLayout.OnTabSelectedListener listener) {
    ...
    /**
     * Add a {@link WTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     *
     * <p>Components that add a listener should take care to remove it when finished via
     * {@link #removeOnTabSelectedListener(WTabLayout.OnTabSelectedListener)}.</p>
     *
     * @param listener listener to add
     */
    public void addOnTabSelectedListener(@NonNull WTabLayout.OnTabSelectedListener listener) {
    ...
    /**
     * Remove the given {@link WTabLayout.OnTabSelectedListener} that was previously added via
     * {@link #addOnTabSelectedListener(WTabLayout.OnTabSelectedListener)}.
     *
     * @param listener listener to remove
     */
    public void removeOnTabSelectedListener(@NonNull WTabLayout.OnTabSelectedListener listener) {
        mSelectedListeners.remove(listener);
    }

    /**
     * Remove all previously added {@link WTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link WTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @WTabLayout.Mode
    ...
    /**
     * The one-stop shop for setting up this {@link WTabLayout} with a {@link ViewPager}.
     * <p>
     ...
     * necessary calls back to the provided {@link WTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<WTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(WTabLayout tabLayout) {
        ...
        public void onPageScrolled(int position, float positionOffset,
            ...
            final WTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
