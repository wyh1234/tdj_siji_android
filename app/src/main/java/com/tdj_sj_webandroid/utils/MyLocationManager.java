...
public class MyLocationManager {
    ...
    public Location beginLocatioon() {
        ...
        if (provider != null) {
            ...
            //不存在位置提供器的情况
            Toast.makeText(activity.getContext(),"不存在位置提供器的情况",Toast.LENGTH_SHORT).show();
        }
        return null;
    }
...
/*    *//**
    ...
    public void getLocationByLonAndLat(Location location, String lat, String lon) {
        ...
        //通过Retrofit2.0进行web请求，进行反地理编码,其实应该进行封装的，这个demo里就直接写了
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(HomepageActivity.BaseUrl)
    ...
    private String judgeProvider(LocationManager locationManager) {
        ...
        if(prodiverlist.contains(LocationManager.NETWORK_PROVIDER)){
            ...
            Toast.makeText(activity.getContext(),"没有可用的位置提供器",Toast.LENGTH_SHORT).show();
        }
        return null;
    }
...
/*    *//**
    ...
    private Map<String, String> getParams(String lat, String lon) {
        ...
        map.put("output","json");
        map.put("pois","1");
        map.put("ak",HomepageActivity.AK);
