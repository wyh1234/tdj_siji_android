...
//        GlideApp.with(mContext).load(path).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.RESOURCE).error(R.mipmap.shibai).dontAnimate().into(mImageView);


    }

    //设置加载中以及加载失败图片
    public static void loadImageViewLodingError(Context mContext, ImageView mImageView) {
//        GlideApp.with(mContext).load(R.mipmap.shibai).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.RESOURCE).into(mImageView);
