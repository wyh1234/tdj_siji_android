...
public class BitmapTools {
     ...
     *
     * @param srcPath （根据路径获取图片并压缩）
     * @return 返回newOpts和bitMap两个对象
     */
    public static ReturnObject getImageTwo(String srcPath) {
        BitmapFactory.Options newOpts = new BitmapFactory.Options();
        // 开始读入图片，此时把options.inJustDecodeBounds 设回true了
        newOpts.inJustDecodeBounds = true;
        Bitmap bitmap = BitmapFactory.decodeFile(srcPath, newOpts);// 此时返回bm为空
        ...
        Bitmap resultBitmap = toTurn(bitmap, readPictureDegree(srcPath));

        return new ReturnObject(newOpts, compressImage(resultBitmap));
    }
    ...
    public static class ReturnObject {
        ...
        public ReturnObject(BitmapFactory.Options options, Bitmap bitmap) {
     ...
     * 在此还原旋转
     */
    public static Bitmap toTurn(Bitmap img, int dg) {
        Matrix matrix = new Matrix();
        matrix.postRotate(+dg); /*翻转90度*/
